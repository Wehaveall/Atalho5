...
// Define the pywebview object and establish the bridge
const api = new pywebview.Api(window);

// Start the PyWebview API
api.start().then(function () {
  // Execute the rest of your JavaScript code that depends on the PyWebview API

  window.addEventListener('DOMContentLoaded', function () {
    // Load the translations when the page loads
    api.load_translations().then(function (translations) {
      // Store the translations in a global variable
      window.translations = translations;

      // Load the current language from the settings
      api.load_language_setting().then(function (language) {
    ...
    for (var i = 0; i < flags.length; i++) {
      flags[i].addEventListener('click', function (event) {
        ...
        api.save_language_setting(window.currentLanguage).then(function () {
          updateLanguage();
        });
      });
    }
