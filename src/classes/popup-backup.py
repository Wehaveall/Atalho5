...
class CustomTkinterPopupSelector:
    def __init__(self, options, key_listener, event):
        ...
        # Add a flag to indicate if a selection has been made
        self.selection_made = False  # <-- New flag
        ...
        self.top_window = ctk.CTkToplevel(
            self.root
        )  # Using customtkinter's CTkToplevel
        ...
        if x is not None and y is not None:
            self.top_window.geometry(
                f"800x500+{x + offset_x}+{y + offset_y}"
            )  # Adjusted to add offsets
    ...
    def make_selection(self, index):
        ...
        self.selection_made = True  # <-- Update flag when a selection is made
    ...
    def on_key(self, event, options):
        try:
            ...
            if 0 <= index < len(options):
                ...
                self.key_listener.popup_open = (
                    True  # Use self.key_listener to access the instance variable
                )
                return
        except ValueError:
    ...
    def get_caret_position(self):
        ...
        threadID = ctypes.windll.user32.GetWindowThreadProcessId(
    ...
    def on_closing(self):
        ...
        if not self.selection_made:  # <-- Check if a selection has been made
            # If no selection was made, reset the word_at_caret or similar variable if needed
            self.key_listener.word_at_caret = ""
